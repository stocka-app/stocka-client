...
export function EditStorageDrawer({
  ...
  const handleFormSubmit = async (values: EditFormValues): Promise<void> => {
    /* istanbul ignore next — !originalValues is unreachable: originalValues is derived from storage (line 253-262) */
  ...
  const handleTypePick = (targetType: StorageType): void => {
    /* istanbul ignore next — unreachable: the drawer short-circuits to a spinner when !storage (line 485-494). */
    if (!storage) return;
    setChangeTypeError(null);
  ...
  const handleRevertTypeChange = (): void => {
    /* istanbul ignore next — unreachable: button only renders when storage + originalValues exist. */
    if (!storage || !originalValues) return;
    setPendingType(null);
    setChangeTypeError(null);
  ...
  const typeLabel = (type?: StorageType): string => {
    ...
    /* istanbul ignore next — unreachable: target is always defined when this branch renders. */
    if (!target) return '';
