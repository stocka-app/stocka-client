...
import { SpaceLimitsSection } from '../SpaceLimitsSection';
...
describe('Given SpaceLimitsSection displays per-type space usage', () => {
  ...
  describe('When the tenant is on FREE tier', () => {
    it('Then the section title is rendered', () => {
      render(<SpaceLimitsSection spaces={activeSpaces} />);
      expect(screen.getByText('limits.title')).toBeInTheDocument();
    });

    it('Then all three space type labels are shown', () => {
      render(<SpaceLimitsSection spaces={activeSpaces} />);
  ...
  describe('When the tenant is on STARTER tier with high space usage', () => {
    ...
    it('Then the progress bar reflects high usage colors', () => {
      const { container } = render(<SpaceLimitsSection spaces={highUsageSpaces} />);
  ...
  describe('When the tenant is on GROWTH tier with medium usage', () => {
    ...
    it('Then the progress bar shows medium usage styling', () => {
      const { container } = render(<SpaceLimitsSection spaces={mediumUsageSpaces} />);
      const progressBars = container.querySelectorAll('[class*="rounded-full"][class*="transition-all"]');
      expect(progressBars.length).toBeGreaterThan(0);
    });
  });
